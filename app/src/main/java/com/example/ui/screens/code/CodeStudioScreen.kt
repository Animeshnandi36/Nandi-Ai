...
class NeuralInferenceEngine(
    ...
    suspend fun streamTokens(prompt: String): Flow<String> = flow {
        println("Dispatching query to ${'$'}modelName on Render Edge...")
    ...
    var codeExplanation by remember {
        mutableStateOf(
            "This reactive stream architecture processes asynchronous neural tokens using Kotlin Coroutine Flow with zero-copy buffer allocations."
        )
    }
                        ...
                        onClick = {
                            if (prompt.isNotBlank()) {
                                scope.launch {
                                    isGenerating = true
                                    delay(800)
                                    val aiPrompt = "Write high quality, production-ready $selectedLang code for: $prompt. Provide the code and a short 2-bullet explanation."
                                    val response = aiRepository.generateChatResponse(listOf("user" to aiPrompt))

                                    // Extract code block
                                    if (response.contains("```")) {
                                        val parts = response.split("```")
                                        if (parts.size >= 2) {
                                            val codePart = parts[1]
                                            val firstLine = codePart.indexOf('\n')
                                            generatedCode = if (firstLine > 0) codePart.substring(firstLine + 1).trim() else codePart.trim()
                                            codeExplanation = if (parts.size >= 3) parts[2].trim() else "Synthesized by NandiAi Code Engine."
                                        }
                                    } else {
                                        generatedCode = "// $selectedLang implementation\n$response"
                                        codeExplanation = "Generated by NandiAi Code Engine."
                                    }
                                    isGenerating = false
                                }
                            }
